...
Citizen.CreateThread(function()
	...
	while true do
		...
		if placedStill and isNearStill then 
			TriggerEvent('vorp:Tip', "[G] Brew Moonshine", 100)
        ...
        if placedBarrel and isNearBarrel and isProcessingMash == false then 
			TriggerEvent('vorp:Tip', "[G] Produce Mash", 100)
			if IsControlJustReleased(0, 0x760A9C6F) then -- g
...
--Moonshine Menu
Citizen.CreateThread(function()
    WarMenu.CreateMenu('still', "Distillery")
    WarMenu.SetSubTitle('still', 'Produce')
    WarMenu.CreateSubMenu('destille', 'still', 'Moonshine')
  
    while true do
        if WarMenu.IsMenuOpened('still') then
            if WarMenu.MenuButton('Produce Moonshine', 'destille') then
            end
            if WarMenu.Button('Pickup the Distillery') then
...
--Mash Menu
Citizen.CreateThread(function()
    WarMenu.CreateMenu('barrel', "Mash Barrel")
    WarMenu.SetSubTitle('barrel', 'Produce')
    ...
    while true do
        if WarMenu.IsMenuOpened('barrel') then
            ...
            if WarMenu.Button('Pickup the Barrel') then
	...
	repeat
		...
		for mash, value in pairs(Config.mashes) do
			if WarMenu.IsMenuOpened(mash) then
                if WarMenu.Button('Prepare') then
            ...
            if WarMenu.IsMenuOpened(moonshine) then
                if isBrewing then
                    if moonshine == actualBrewingMoonshine then
                        ...
                        if WarMenu.Button('Brewing Moonshine: '..time..'min') then
                            WarMenu.CloseMenu()
                        end
                    end
                elseif getMoonshineMenu then
                    if moonshine == actualBrewingMoonshine then
                        if WarMenu.Button(value.Label..' tap') then
                            TriggerServerEvent("moonshiner:giveMoonshine", moonshine, value.minXP, value.maxXP)
                            TriggerEvent("vorp:TipBottom", "You have finished brewing Moonshine", 3000)
                    ...
                    if WarMenu.Button(value.Label..' prepare') then
...
AddEventHandler('moonshiner:placeProp', function(propName)
    ...
    while placing do
        ...
        if PromptHasHoldModeCompleted(BuildPrompt) then
            ...
            exports['progressBars']:startUI(6000, "Building...")
            Citizen.Wait(6000)
...
AddEventHandler('moonshiner:deleteProp', function(propName)
    ...
    if isNearStill then
        ...
        exports['progressBars']:startUI(5000, "Picking up...")
        Citizen.Wait(5000)
...
AddEventHandler('moonshiner:harvest', function(values)
    ...
    TaskStartScenarioInPlace(playerPed, GetHashKey('WORLD_HUMAN_CROUCH_INSPECT'), -1, true, false, false, false)
    exports['progressBars']:startUI(15000, "Harvest...")
    Citizen.Wait(15000)
    ClearPedTasksImmediately(playerPed)
...
AddEventHandler('moonshiner:startMash', function(mash, itemArray, time, minXP, maxXP, message)
    ...
    TriggerEvent("vorp:ExecuteServerCallBack", "check_mashItems", function(canMash)
        if canMash then
            ...
            TaskStartScenarioInPlace(playerPed, GetHashKey('WORLD_HUMAN_CROUCH_INSPECT'), -1, true, false, false, false)
            exports['progressBars']:startUI(mashTime, "Mashing ingredients...")
            Citizen.Wait(mashTime)
            ClearPedTasksImmediately(playerPed)

            TriggerServerEvent("moonshiner:giveMash", mash, minXP, maxXP)
            TriggerEvent("vorp:TipBottom", "You have finished making "..mash, 4000)
    ...
    TriggerEvent("vorp:ExecuteServerCallBack", "check_moonshineItems", function(canBrew)
        if canBrew then
            ...
            if isBrewing == false then
                ...
                isBrewing = true
                startTimer()
                TriggerEvent("vorp:TipBottom", "Brewing: "..round(((actualBrewingTime - timeToBrew) / 60), 2).." minutes remaining", 3500)
            else
                TriggerEvent("vorp:TipBottom", "You are already brewing Moonshine!", 3000)
            end
        else
            TriggerEvent("vorp:TipBottom", "You are missing ingredients", 3000)
        end
    end, itemArray)
end)
--############################## END Events ##############################--
...
function startTimer()
    ...
    Citizen.CreateThread(function()      
        while timeToBrew <= actualBrewingTime do
            ...
            print("Seconds Remaining: "..timeToBrew)
        end
        isBrewing = false
        getMoonshineMenu = true
        TriggerEvent("vorp:TipBottom", "The moonshine is ready!", 4000)
    end)
end
